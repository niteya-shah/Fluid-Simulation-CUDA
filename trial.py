...
__device__ void pressure_force(float *press_force, const float *mass,const float mag,
  ...
  float const_val =
      *mass * *k * (density_n[blockIdx.x] + density_n[i] - 2 * *rest_density) * 1/(2 * density_n[i]);
...
__device__ void color_field_grad(float *color_field_grad_val, const float *mass,
                                 const float *density_n, float *r_dash,const int *neighbors,
  ...
  color_field_grad_val[blockIdx.x * blockDim.x * 3 + 3 * threadIdx.x] = grad_Wpoly[0] * inner_val;
  color_field_grad_val[blockIdx.x * blockDim.x * 3 + 3 * threadIdx.x + 1] = grad_Wpoly[1] * inner_val;
  color_field_grad_val[blockIdx.x * blockDim.x * 3 + 3 * threadIdx.x + 2] = grad_Wpoly[2] * inner_val;
}

__device__ void color_field_lap(float *color_field_lap_val, const float *mass,
                                const float *density_n, const float *r,const int *neighbors,
                                const float *h, const float mag,
                                const float *lap_WPoly6_const) {
  const int i = blockIdx.x * blockDim.x + threadIdx.x;
  color_field_lap_val[i] =
      *mass * 1 / (density_n[neighbors[i]] +  0.0000001f) * lap_WPoly6(h, mag, lap_WPoly6_const);
}
...
__global__ void calc_forces(
    ...
    const float *r1,const float *density_n,float *vel_n, int *neighbors,
  ...
  color_field_grad(color_field_grad_val, mass, density_n, r_dash,neighbors ,h, mag,
                   grad_WPoly6_const);
  color_field_lap(color_field_lap_val, mass, density_n, r_dash,neighbors ,h, mag,
                  lap_WPoly6_const);
  force[blockIdx.x * blockDim.x * 3 + 3 * i] = clamp(press_force[0] + visc_force[0], 200, -200);
  force[blockIdx.x * blockDim.x * 3 + 3 * i + 1] = clamp(press_force[1] + visc_force[1], 200, -200);
  force[blockIdx.x * blockDim.x * 3 + 3 * i + 2] = clamp(press_force[2] + visc_force[2], 200, -200);
  }
  else
  {
...
for iteration in range(100):
    ...
    neighbors = nn.kneighbors(r1, return_distance=False).flatten().astype(np.int32)
    ...
    force = np.zeros([const.num_particles,const.num_neighbhours,3]).astype(np.float32)
    color_field_lap_val = np.zeros([const.num_particles, const.num_neighbhours]).astype(np.float32)
    color_field_grad_val = np.zeros([const.num_particles, const.num_neighbhours, 3]).astype(np.float32)
    ...
    calc_forces(drv.Out(force), drv.Out(color_field_lap_val), drv.Out(color_field_grad_val), drv.In(r_dash),  drv.In(Density), drv.In(v_n), drv.In(neighbors) ,drv.In(const.h), drv.In(const.eta), drv.In(const.mass), drv.In(const.rest_density), drv.In(const.k),  drv.In(const.grad_WPoly6_const),drv.In(const.lap_WPoly6_const),drv.In(const.Wspiky_const),drv.In(const.grad_Wspiky_const),drv.In(const.Wviscosity_const),drv.In(const.lap_Wviscosity_const), block=(const.num_neighbhours,1,1), grid=(const.num_particles,1))
    force = force.reshape([const.num_particles, const.num_neighbhours, 3]).sum(axis = 1)
    color_field_grad_val = color_field_grad_val.reshape([const.num_particles, const.num_neighbhours, 3]).sum(axis = 1)
    color_field_lap_val = color_field_lap_val.reshape([const.num_particles, const.num_neighbhours]).sum(axis = 1)

    force = force.flatten().astype(np.float32)
    color_field_grad_val = color_field_grad_val.flatten().astype(np.float32)

    update_pos(drv.InOut(r_dash), drv.InOut(v_n), drv.In(force), drv.In(const.threshold),drv.In(const.mass),drv.In(const.time) ,drv.In(const.sigma),drv.In(const.Width),drv.In(const.damping),drv.In(const.eps) ,drv.In(color_field_lap_val), drv.In(color_field_grad_val), block=(1,1,1), grid=(const.num_particles,1))
